...
- Aideon Desktop is the primary UX shell for the desktop product. All workspaces (Praxis Canvas and future tools) render inside this shell instead of shipping their own chrome.
...
- **Centre workspace:** hosts the active workspace surface (Praxis Canvas initially).
...
- Left tree shows projects/workspaces using the design-system sidebar menus. Initial stub data lives in `DesktopTree`; replace with adapter-fed data when available.
...
- `main` – workspace surface (Praxis Canvas surface today).
...
- Tauri loads the `app/AideonDesktop` bundle (`index.html` → `src/main.tsx` → `AideonDesktopRoot`); PraxisCanvas surfaces mount inside the centre slot rather than owning the window.
