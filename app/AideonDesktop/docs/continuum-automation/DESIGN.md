...
- Continuum automation DTOs live in `app/AideonDesktop/src/dtos/continuum.ts` and are re-exported
  from `app/AideonDesktop/src/dtos/index.ts`.
- The automation surface calls the host exclusively via an adapter surface under
  `app/AideonDesktop/src/workspaces/continuum/` (or an equivalent workspace module).
