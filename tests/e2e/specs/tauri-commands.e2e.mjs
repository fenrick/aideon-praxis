...
// Assert the Tauri invoke bridge is exposed on the main window before any
// command is dispatched over it.
async function assertTauriBridgeReady() {
...
// System/setup surface: worker health, logging + metrics (raw commands that
// bypass the IpcRequest/IpcResponse envelope), setup state, and window open.
async function assertSystemCommands() {
  ...
  assertOk(await invokeIpc('system_setup_complete', { task: 'frontend' }), 'system_setup_complete');
  assertOk(await invokeIpc('system_window_open', { window: 'settings' }), 'system_window_open');
}

// Workspace project/template surface: project listing, template listing (with a
// default-templates assertion) and template save.
async function assertWorkspaceTemplateCommands(ids) {
...
// List commits over the bridge and resolve the latest commit id (falling back
// to 'main'). Used as the asOf anchor for canvas/artefact/temporal commands.
async function resolveLatestCommitId() {
  const commits = assertOk(
    await invokeIpc('chrona_temporal_list_commits', { branch: 'main' }),
    'chrona_temporal_list_commits',
  );
  return Array.isArray(commits) && commits.length > 0 ? (commits.at(-1)?.id ?? 'main') : 'main';
}
...
// Artefact execution surface: graph, catalogue, matrix and chart artefacts.
async function assertArtefactCommands(latestCommitId) {
  assertOk(
...
// Task application and scenario listing surface.
async function assertTaskAndScenarioCommands(ids) {
  assertOk(
...
// Temporal query + mutation surface: state-at, diff, list branches, topology
// delta, commit changes, and branch create/merge (ok-or-error for mutations).
async function assertTemporalCommands(latestCommitId) {
...
// Workspace lifecycle surface: create/status/rebuild/close/open. Independent of
// the in-process WorkerState used by chrona/praxis, so these calls do not affect
// the engine commands above. workspace_create writes a real workspace to /tmp;
// ok-or-error is used because create may fail in constrained CI environments.
async function assertWorkspaceLifecycleCommands() {
  assertOkOrError(
...
async function assertWorkspaceAuthoringCommands() {
...
async function assertManifestParity() {
  // fixed in-repo manifest path, no external input
  // nosemgrep
...
describe('tauri e2e command coverage', () => {
  it('executes all IPC commands over the tauri bridge', async () => {
    ...
    await assertSystemCommands();
    await assertWorkspaceTemplateCommands(ids);
    ...
    await assertWorkspaceLifecycleCommands();
    await assertWorkspaceAuthoringCommands();

    await assertManifestParity();
  });
});
