...
describe('templates', () => {
  it('instantiates templates with default timestamps when asOf is omitted', () => {
    const template = BUILT_IN_LAYOUTS[0];
    expect(template).toBeDefined();
    if (!template) {
      return;
    }
    const widgets = instantiateLayout(template, {});
  ...
  it('instantiates templates with provided asOf context', () => {
    ...
    const widgets = instantiateLayout(template, { asOf: 'c1', scenario: 'main' });
    for (const widget of widgets) {
  ...
  it('captures runtime widgets into a reusable template', () => {
    ...
    const widgets = instantiateLayout(template, { scenario: 'main' });
    const captured = captureLayoutFromWidgets('Snapshot', 'Saved layout', widgets);
