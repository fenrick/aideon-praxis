...
**`ready_read_write` asserts foundation read/write readiness only.** It means the workspace lifecycle is open, the derived foundation runtime is rebuilt or current, and the host may accept foundation-scoped reads and writes. It does **not** assert that the effective schema has compiled (M1), that semantic validation is available (M1), or that temporal resolution is available (M2). The M0 status is deliberately a **flat lifecycle state**, not a multi-axis readiness object — M0 must not name axes it cannot populate or prove. When M1 and M2 exist, they add their own readiness/proof as **additive sibling fields** (`effective_schema?: { state, effective_schema_hash? }`, `temporal_resolution?: { state, temporal_probe_hash? }`), never by restructuring or renaming the lifecycle `state` — additive under [ADR-0017](../../06-adrs/ADR-0017-contract-and-dto-versioning.md), so consumers ignore unknown optional fields. The proof-carrying readiness of M0 ([ADR-0040](../../06-adrs/ADR-0040-m0-host-validation-gate-and-proof-carrying-readiness.md)) is the foundation axis; later axes are added beside it.

> **Event-manifest follow-up.** `workspace_opened`/`workspace_closed` are prose-only today and absent from [`event-manifest.json`](../../contracts/event-manifest.json). Resolve the gap by defining **one** typed `workspace.lifecycle.changed` Rust event and generating it into the manifest, rather than leaving prose-only events. If `workspace_opened` is retained, it is defined narrowly: emitted only on reaching a terminal usable open state, with an explicit `mode` of read-write or recovery read-only.
