...
| `aideon_hlc_state`      | Per-partition HLC watermark; PK `partition_id`, `last_hlc`. The monotonicity anchor for `Hlc::now()` ([bitemporal-and-hlc](./bitemporal-and-hlc.md)). |
...
| Table                      | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `aideon_entities`          | Entity rows; PK `(partition_id, entity_id)`, scenario overlay via `scenario_id`; `entity_kind`, `type_id` (nullable), `is_deleted`, `acl_group_id` / `owner_actor_id` / `visibility` (nullable, **unused at M0** — no canonical op writes them and no read consults them; record-level access control is omitted at M0 and owned by Themis/M6, [ADR-0030](../../06-adrs/ADR-0030-governance-themis.md)), `created_op_id`, `updated_op_id`, `created_asserted_at_hlc`, `updated_asserted_at_hlc`. |
...
| `aideon_fields`                 | Field definitions; PK `(partition_id, field_id)`; `label`, `value_type`, `cardinality`, `merge_policy`, `is_indexed`, `disallow_overlap`, `is_deleted`, `updated_asserted_at_hlc`.                          |
