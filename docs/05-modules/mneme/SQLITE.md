...
| `aideon_fields`                 | Field definitions; PK `(partition_id, field_id)`; `label`, `value_type`, `cardinality`, `is_indexed`, `is_deleted`, `updated_asserted_at_hlc`. No `merge_policy` column (convergence is M6, [ADR-0034](../../06-adrs/ADR-0034-merge-correctness-and-convergence.md)) and no `disallow_overlap` column (the slot-level temporal-overlap constraint is an M2 write-time rule, not M0 metadata). |
| `aideon_type_fields`            | Field bindings to types; PK `(partition_id, type_id, field_id)`; `is_required`, `override_default`, `tighten_required`, per-type default columns, `updated_asserted_at_hlc`. No `disallow_overlap` column (M2, see `aideon_fields`).                                                                                                                                                          |
