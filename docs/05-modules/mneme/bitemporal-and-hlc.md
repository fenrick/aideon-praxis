...
`Hlc::now()` in `mneme_core::time` produces the next HLC by advancing a stored last-HLC atomically; `Hlc::physical_micros()` strips the counter to recover the wall-clock reading for display.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| **Normal advance**                  | `Hlc::now()` returns `max(previous_hlc, physical_now)`; on a physical tie it increments the counter.                                       | A later call never sorts before an earlier one.                                                                      |
...
| **Counter overflow**                | 12 bits admit 4096 assertions in one microsecond; if exhausted, the physical component advances by one microsecond and the counter resets. | Order and monotonicity preserved, at the cost of a sub-microsecond drift ahead of wall time. Degenerate in practice. |
