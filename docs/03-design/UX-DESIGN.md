...
- **Navigation-rail contents and ordering** — the rail's job is fixed ([the-shell.md](./the-shell.md), [hig/shell-and-navigation.md](./hig/shell-and-navigation.md)), but the actual entries (workspaces, scenarios, artefact families, pinned/recent, engine presence) and their arrangement are not pinned.
- **Surface and widget launch / discovery** — how a user opens a surface or adds an engine widget to the content area (toolbar menu? library dialog? "+"?) is described as a _pattern_ ([../frontend/shell.md](../frontend/shell.md)) but not as an IA.
