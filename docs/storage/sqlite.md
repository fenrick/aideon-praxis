...
- **Engine**: SQLite 3 in WAL mode with `synchronous=NORMAL` and `foreign_keys=ON` via the
  `aideon-mneme-core` crate (`crates/aideon-mneme-core`).
...
- Implemented with `rusqlite_migration`; see `run_migrations` in `crates/aideon-mneme-core/src/sqlite.rs`.
...
(`crates/aideon-praxis-host/src/worker.rs`) creates the directory and opens the DB at startup via
