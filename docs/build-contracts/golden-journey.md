...
- **Oracle — the journey's final assertion:** re-running steps 5–7 after the rebuild produces results **semantically equivalent** to before — identical resolved facts and identical catalogue rows — proven by a deterministic hash over the canonical serialisation of each result ([ADR-0027](../06-adrs/ADR-0027-projection-consistency-model.md), rebuild-equivalence). Equivalence is _not_ byte-identical runtime state; it is identical resolved meaning. _(oracle: planned — Increment 2 defines the equivalence hash.)_
