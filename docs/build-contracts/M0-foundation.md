...
> **This is a build specification for a clean, design-led build — not a description of the current code.** M0 is built **to this spec**: the live `model/ops/` segment writer and reader, the workspace `manifest.json`, the writer lock, the integrity and recovery rules, the content-addressed blob boundary, the typed canonical operation record, and the rebuild pipeline. An earlier inside-out prototype exists in the tree (SQLite-as-store, `Vec<u8>` payloads, untyped value variants, no `model/ops/` writer); it does **not** satisfy [ADR-0001](../06-adrs/ADR-0001-workspace-is-canonical-authority.md)/[ADR-0002](../06-adrs/ADR-0002-portable-workspace-format.md) and is **deleted and recreated** to this spec as the first step of the M0 build ([#292](https://github.com/aideon-ai/aideon-desktop/issues/292)) — git preserves it, so nothing is lost. SQLite remains the **derived** runtime cache behind the storage trait ([ADR-0004](../06-adrs/ADR-0004-storage-engine-abstraction.md), [SQLITE](../05-modules/mneme/SQLITE.md)), a projection of the canonical op log, never the authoritative store.
...
| Serialisation       | Versioned canonical JSON; full-range coords as decimal strings                                                                                                                                                       | —                        | [canonical-json](../04-contracts/canonical-json.md); ADR-0038                                                                                                                      |
