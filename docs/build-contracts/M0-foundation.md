...
| Host typed workspace-lifecycle IPC + capability + rebuild-trigger | `src-tauri` · `workspace_lifecycle.rs`, `bindings.rs`, `capabilities/`, `permissions/`       | `workspace_lifecycle::tests::*` (create/open/status/close/rebuild through real IPC dispatch); `security_posture::{read_and_mutating_permissions_partition_the_generated_surface,mutating_commands_are_granted_only_to_the_main_window}` — read ∪ mutating == generated surface, mutating → `main` only ([#290](https://github.com/aideon-ai/aideon-desktop/issues/290), [#318](https://github.com/aideon-ai/aideon-desktop/issues/318)) | ✅     |
| **Host lifecycle wired to canonical file path** (M0 blocker)      | `src-tauri` · `workspace_lifecycle.rs`; `crates/engine`, `crates/mneme_store`                | create → author → rebuild → `foundation_rebuild_hash` equivalence via canonical files (not SQLite): `workspace_lifecycle::tests::{author_node_then_list_survives_reopen_with_hash_equality,delete_runtime_externally_and_reopen_rebuilds_deterministically}`, `aideon_engine::tests::full_thread_rebuild_is_structurally_and_semantically_equivalent` ([#254](https://github.com/aideon-ai/aideon-desktop/issues/254))                  | ✅     |
| Rebuild as **accepted work** + readiness lifecycle events         | `src-tauri` · `jobs.rs`, `workspace_lifecycle.rs`                                            | `workspace_lifecycle::tests::rebuild_runs_as_accepted_work_and_readiness_carries_the_pre_wipe_hash` (proof-carrying readiness, [ADR-0040](../06-adrs/ADR-0040-m0-host-validation-gate-and-proof-carrying-readiness.md)); `concurrent_rebuild_is_refused_with_backpressure` ([#316](https://github.com/aideon-ai/aideon-desktop/issues/316))                                                                                             | ✅     |
| RFC-9457 error catalogue + failure mapping                        | `mneme_store` · `error.rs`; `src-tauri` · `ipc.rs`                                           | `ipc::tests::ipc_response_err_serializes_rfc9457_problem_detail` (type/code/title/detail/category/recovery/correlationId); `classify` catalogue + `map_store_error`/`host_error` mappings ([#287](https://github.com/aideon-ai/aideon-desktop/issues/287), [#270](https://github.com/aideon-ai/aideon-desktop/issues/270))                                                                                                              | ✅     |
| Aideon Desktop shell + design-system proxies                      | `src/` · `design-system/src/desktop-shell/`, `root.tsx`, `DesktopTree.tsx`, `canvas/app.tsx` | Storybook stories + visual inspection (light + dark, representative widths); no raw `shadcn` / `react-resizable-panels` primitives ([DESIGN.md](../03-design/DESIGN.md))                                                                                                                                                                                                                                                                | ✅     |
| Operation-schema → Rust DTO drift check in CI                     | `crates/mneme_core` · `tests/schema_drift.rs`; `src/ops.rs`                                  | `schema_drift::*` fails when a JSON schema in `docs/contracts/operations/` diverges from the corresponding Rust DTO — auto-discovers op-kind schemas, no manual registration ([#371](https://github.com/aideon-ai/aideon-desktop/issues/371))                                                                                                                                                                                           | ✅     |
