...
| Derived runtime + rebuild + `foundation_rebuild_hash`                                                                | M0    | `m0_exit` wipe→rebuild equivalence                       | ✅     |
| Workspace lifecycle + single-writer lock                                                                             | M0    | in-window e2e (lifecycle) + crate tests                  | ◐      |
| Minimal accepted-work core (job runner, RunEvent, readiness, backpressure) [D1]                                      | M0    | in-window e2e (rebuild = accepted work)                  | ☐      |
| Typed IPC surface + capability enforcement + **codegen** manifest [D7]                                               | M0    | host boundary e2e (round-trip, per-window deny, drift)   | ☐      |
| Shell chrome + lifecycle/honest-state UX [D5]                                                                        | M0    | **in-Tauri-window e2e** (composition + interaction) [D2] | ☐      |
