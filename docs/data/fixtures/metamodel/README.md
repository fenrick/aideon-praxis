...
| ---------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `MALFORMED_PROPS`            | node, edge | the attribute set is not a JSON object (e.g. a bare number, string, or array where an object is required).                  | validation-rules §node 1                             |
| `UNKNOWN_TYPE`               | node       | `type` does not match a known type descriptor.                                                                              | validation-rules §node 2                             |
| `UNKNOWN_RELATIONSHIP_TYPE`  | edge       | relationship `type` does not match a known relationship descriptor.                                                         | validation-rules §edge 1                             |
| `MISSING_REQUIRED_ATTRIBUTE` | node, edge | a required attribute (e.g. `name`, `accesses.mode`, `plan_effect.op`/`target_ref`) is absent or null.                       | validation-rules §node 4, §edge 6                    |
| `WRONG_ATTRIBUTE_KIND`       | node, edge | a value does not match its declared kind (e.g. a string where `number` is required and cannot be coerced).                  | validation-rules §node 3, §edge 6                    |
| `STRING_TOO_LONG`            | node, edge | a `string` value exceeds 256, or a `text` value exceeds 4096.                                                               | validation-rules §global, §node 5                    |
| `ENUM_VALUE_NOT_ALLOWED`     | node, edge | an `enum` value is not a declared variant (case-insensitive match), e.g. `tier = "Tactical"`.                               | validation-rules §node 6, §edge 6                    |
...
| `MULTIPLICITY_EXCEEDED`      | edge       | a `one`-bounded endpoint already carries an edge of this relationship type; the seed is all `many-many` (never triggered).  | validation-rules §edge 5                             |
