...
- **The `Stage` supertype gap.** `ValueStreamStage` declares `extends: Stage`, but no `Stage` type exists in the seed. The fixture sets `extends_resolved: false` and records the gap in a `gap` block; the effective schema is `ValueStreamStage`'s own three slots only. The reconciliation (add `Stage`, or drop the `extends`) is owned by the metamodel design, not by this fixture ([data/README, the `Stage` gap](../../README.md), [slots-and-effective-schema honest-state note](../../../03-design/metamodel/slots-and-effective-schema.md)).
- **UUID minting is not reproducible from the seed.** UUIDs are stated to be UUIDv5 over a fixed project namespace plus a stable name path ([packages-and-registry](../../../03-design/metamodel/packages-and-registry.md)), but `core-v1.json` does not record the namespace UUID. The fixtures therefore **read** every UUID from the seed; they cannot recompute one. The namespace is design-intent until the compiler fixes it.
