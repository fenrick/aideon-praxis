...
even when the local wall clock is behind the canonical maximum, the maximum HLC has counter `4095`, or an imported `asserted_at` is future-dated. The HLC oracle cases cover: normal rebuild then a strictly-later op; wall clock behind the canonical max; max HLC at counter `4095`; a future-dated imported assertion; an empty workspace (unset watermark, first op at `pack(physical_now, 0)`); duplicate replay records; an op that minted an HLC then failed before append (the advancement is discarded); two open workspaces with independent clocks; and checked overflow at the maximum representable `i64` (explicit clock-exhaustion error, never a wrap).

**The open-time rebuild decision turns on a derived replay frontier (`ReplayHead`).** The runtime persists, per partition, the canonical position replayed so far as a **stable logical segment coordinate** (`segment_seqno` + `byte_offset` + `applied_record_count` + `last_record_digest`), and **advances it in the same transaction** as the required projection apply — so after a crash the projection changes and the cursor advance either both committed or neither did. On open the host compares a validated `ReplayHead` to the canonical tail: **absent/unusable → full rebuild**; **current (`==` tail, and structure + projection-version checks pass) → no replay**; **strict prefix → incremental replay of `(ReplayHead, tail]` only**; **ahead/inconsistent → discard and full rebuild** (never repair backwards). Full specification in [workspace-integrity-and-recovery](../../../05-modules/mneme/workspace-integrity-and-recovery.md#open-time-rebuild-decision-and-the-replay-frontier); host-side lifecycle states and the readiness event in [workspace-lifecycle](../../../05-modules/host/workspace-lifecycle.md). The lifecycle/replay oracle cases cover: runtime absent → full rebuild; runtime current → no replay job; one durable op beyond head → incremental replay only; crash between canonical append and projection commit → tail replay repairs the runtime; crash inside the projection transaction → cursor and projections both remain behind; cursor ahead of tail → discard and rebuild; cursor at a non-record boundary → discard; loose segment sealed between sessions → the logical cursor stays valid; rebuild failure → `recovery_read_only`, canonical files unchanged; renderer restart after readiness → current lifecycle state supplied without polling; a new write after replay sorts after all canonical history; and a second reopen with an unchanged tail performs no unnecessary rebuild.
