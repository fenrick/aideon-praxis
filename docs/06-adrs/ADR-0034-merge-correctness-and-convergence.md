...
- **The CRDT operation kinds and merge policies are owned here, and are not earlier than M6.** `or-set-update` / `counter-update` (op codes 6/7) and the `or-set` / `counter` / `text` merge policies carry convergence semantics — dedup, commutativity, reset, grow-only-vs-PN counter, observed-add tags for the OR-set, a sequence model for collaborative text — that only this ADR's contract settles. They are therefore removed from the M0 operation schemas (codes reserved, an M0 reader refuses a workspace that requires them, [ADR-0038](./ADR-0038-canonical-operation-record-identity-and-commit-protocol.md)) rather than accepted with under-specified semantics. Slot **cardinality** declarations (single- vs multi-valued shape) enter at M1 and temporal resolution at M2; the convergence/merge-policy axis itself does **not** exist in the MVP metamodel and is added additively here at M6 — an absent policy means the MVP single-writer resolver, so existing slots need no backfill when it lands.
