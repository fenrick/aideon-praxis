...
- **Monotonicity is guaranteed by advancing a stored last-HLC.** `Hlc::now()` advances a global last-HLC atomically: the new HLC is the maximum of (the previous HLC) and (the current physical microsecond), with the counter incremented on a tie. A later call therefore never returns a value that sorts before an earlier one, even if the wall clock has moved backwards.
