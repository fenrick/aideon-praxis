...
- Praxis loads the document (plus optional overrides) into `MetaModelRegistry`
  (`crates/aideon-praxis-engine/src/meta.rs`). Every change-set touches the registry before a snapshot mutates,
...
- The initial meta-model is seeded via `crates/aideon-praxis-engine/src/meta_seed.rs`, which translates the
...
- `cargo aideon-xtask import-dataset` replays the dataset into any datastore (use `--dry-run` for CI), and
