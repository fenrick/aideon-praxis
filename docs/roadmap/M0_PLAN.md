...
- Host design (capabilities, lifecycle, windowing, jobs): `src-tauri/DESIGN.md`
...
1. [x] Ensure first-run setup is a host-owned flow with explicit lifecycle phases (backend setup,
       frontend ready ack), not renderer-side hidden defaults. Refs: `src-tauri/DESIGN.md`,
...
2. [~] Define and document the **first-run state machine** (states, transitions, failure modes,
   retry points) and keep the UI aligned to those states. Refs: `src-tauri/DESIGN.md`,
...
4. [x] Ensure all first-run required assets are packaged locally (no CDN/runtime downloads); verify
       `next export` output is sufficient for Tauri boot. Refs: `DESIGN.md`,
...
5. [x] Define “first-run required storage” explicitly (paths, minimum schema, seeded data) and
       ...
       `src-tauri/DESIGN.md`, `crates/mneme_store/DESIGN.md`, `docs/storage/SQLITE.md`.
6. [x] Add a dedicated “Setup/Status” UX path for setup failures that avoids crash loops and
       supports retry/copy diagnostics. Refs: `docs/UX-DESIGN.md`, `DESIGN.md`,
       `src-tauri/DESIGN.md`.
...
8. [x] Ensure first-run seeding is deterministic and idempotent (safe to re-run); record what was
       ...
       surfaces dataset/ metamodel versions). Refs: `crates/mneme_store/DESIGN.md`, `src-tauri/DESIGN.md`.
9. [x] Provide a “factory reset” / “safe wipe” action gated by explicit confirmation and
       capabilities (for development and recovery). Refs: `src-tauri/DESIGN.md`,
...
1. [x] Keep the renderer untrusted: all privileged actions flow through host IPC (commands/events)
       and capability checks. Refs: `ARCHITECTURE-BOUNDARY.md`, `src-tauri/DESIGN.md`.
...
7. [ ] Confirm logs/diagnostics do not leak sensitive data by default; require redaction on any
       user-facing “copy diagnostics” flows. Refs: `src-tauri/DESIGN.md`, `docs/UX-DESIGN.md`.
...
10. [x] Add a short “security invariants” section to module UX docs for any workspace that triggers
        ...
        `docs/frontend/praxis-workspace/DESIGN.md`, `src-tauri/DESIGN.md`.
...
1. [x] Require snake_case command identifiers (no dots) for all invoke surfaces. Refs:
       `docs/CONTRACTS-AND-SCHEMAS.md`, `src-tauri/DESIGN.md`, `ARCHITECTURE-BOUNDARY.md`.
2. [x] Standardize request/response envelope shape (request id, status, result/error). Refs:
       `docs/CONTRACTS-AND-SCHEMAS.md`, `src-tauri/DESIGN.md`.
3. [x] Standardize `HostError` shape (`code`, `message`, optional `details`) and ensure it is stable
       across commands. Refs: `src-tauri/DESIGN.md`, `docs/CONTRACTS-AND-SCHEMAS.md`.
...
7. [x] Document versioning policy for commands/events (additive changes, deprecation windows,
       schemaVersion bumps). Refs: `docs/CONTRACTS-AND-SCHEMAS.md`, `src-tauri/DESIGN.md`.
...
10. [ ] Ensure all naming across IPC, events, capabilities, and job identifiers remains consistent
        (snake_case only). Refs: `ARCHITECTURE-BOUNDARY.md`, `src-tauri/DESIGN.md`,
        `docs/CONTRACTS-AND-SCHEMAS.md`.
...
3. [x] Ensure database location follows platform conventions (app data directories), not repo-relative
       paths. Refs: `src-tauri/DESIGN.md`, `ARCHITECTURE-BOUNDARY.md`.
4. [ ] Add an explicit “migration report” surfaced in the Status window (what ran, versions, any
       warnings) to support recovery. Refs: `docs/UX-DESIGN.md`, `src-tauri/DESIGN.md`.
...
6. [ ] Provide a backup/restore mechanism at the workspace boundary (not raw DB copy unless
       explicitly safe), and document it. Refs: `src-tauri/DESIGN.md`, `docs/UX-DESIGN.md`.
...
9. [ ] Add “storage health” checks surfaced via host diagnostics (disk full, DB locked, corruption
       hints). Refs: `src-tauri/DESIGN.md`, `docs/UX-DESIGN.md`.
10. [ ] Document how “workspace identity” maps to storage partitions and how multiple workspaces are
        isolated in storage. Refs: `src-tauri/DESIGN.md`, `crates/mneme_core/DESIGN.md`.
...
1. [x] Define the canonical window set and their responsibilities (including Status as the recovery
       and diagnostics surface). Refs: `src-tauri/DESIGN.md`, `DESIGN.md`.
...
3. [x] Ensure menus/accelerators dispatch into the renderer via a snake_case event (`shell_command`)
       with snake_case command ids. Refs: `docs/CONTRACTS-AND-SCHEMAS.md`, `src-tauri/DESIGN.md`.
...
1. [x] Ensure workspace seed data is host-owned (projects/scenarios/templates are not stored in the
       renderer). Refs: `ARCHITECTURE-BOUNDARY.md`, `src-tauri/DESIGN.md`.
...
4. [ ] Define a persistent “workspace identity” model (ids, display names, last opened, pinned)
       and make navigation deterministic. Refs: `src-tauri/DESIGN.md`, `docs/UX-DESIGN.md`.
5. [ ] Define the scenario lifecycle UX even for M0: list, select, create, delete (or explicit
       ...
       `src-tauri/DESIGN.md`, `crates/chrona/DESIGN.md`.
6. [ ] Add loading/error/empty states for seed/navigation flows (especially “no projects yet” and
       ...
       `docs/frontend/praxis-workspace/DESIGN.md`, `docs/TESTING-STRATEGY.md`.
7. [ ] Ensure template seeding is idempotent and versioned (upgrade path for default templates).
       Refs: `src-tauri/DESIGN.md`, `crates/mneme_store/DESIGN.md`.
8. [ ] Ensure navigation supports recovery mode when seed data is unavailable (offer restore or
       create new workspace). Refs: `docs/UX-DESIGN.md`, `src-tauri/DESIGN.md`.
...
10. [ ] Define and document the future “module workspace switcher” behaviour (Praxis ↔ Mneme ↔
        ...
        `DESIGN.md`, `docs/UX-DESIGN.md`, `docs/DESIGN.md`.
...
3. [ ] Implement “backup” as a user-visible workflow with bounded, explainable behaviour (what is
       included, redaction posture, where saved). Refs: `docs/UX-DESIGN.md`, `src-tauri/DESIGN.md`,
       `ARCHITECTURE-BOUNDARY.md`.
...
7. [ ] Implement safe concurrency rules (prevent double-open, handle multi-window attempts) and
       make them testable. Refs: `src-tauri/DESIGN.md`.
8. [ ] Provide a “workspace lock” / “in use” story (especially for future remote parity), but
       ...
       `src-tauri/DESIGN.md`, `docs/DESIGN.md`.
...
10. [ ] Update module UX docs to include lifecycle UX flows (navigation + Status + recovery) with
        explicit empty/loading/error states. Refs: `DESIGN.md`,
        `docs/frontend/praxis-workspace/DESIGN.md`, `docs/UX-DESIGN.md`.
...
1. [ ] Define the “cold start” benchmark method (what is measured, on what hardware class, with
       what dataset) and make it repeatable. Refs: `docs/ROADMAP.md`, `src-tauri/DESIGN.md`.
2. [ ] Ensure cold start SLO is measured end-to-end (process start → main window interactive), not
       only “backend ready”. Refs: `docs/ROADMAP.md`, `docs/UX-DESIGN.md`, `src-tauri/DESIGN.md`.
