...
A **surface** is a platform-owned, navigable work destination (the eight in [ux/workspace-family.md](../03-design/ux/workspace-family.md)); a **widget** is a content component rendered _within_ a surface. Navigation selects a surface; `PlatformContent` renders that **active surface instance** — its composition and layout — not one undifferentiated workspace-wide widget canvas. A surface definition declares at least: `surface_id`, `label`, `purpose`, availability requirements, default composition, allowed widget contributions, toolbar contribution, inspector behaviour, empty-state behaviour, layout-persistence key, and **viewpoint policy** (a surface owns a policy, not a hard default that overrides the shell on every navigation — see [hig/shell-and-navigation.md](../03-design/hig/shell-and-navigation.md)).

Composability is **per-surface**, not universal: the modelling studio is extensively composable; the scenario studio is bounded (comparison/timeline/explanation); workspace home, the artefact library, review, briefing, import, and administration are platform-composed and not open dashboards. Widgets are a reusable composition primitive, **not** a promise that every surface is an editable canvas.
