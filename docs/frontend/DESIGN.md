...
This document is superseded by the decomposed renderer architecture. Its content — the one platform-owned shell, its four regions, how engines contribute widgets, and the static-export constraints — now lives in [shell.md](./shell.md), and the cross-cutting narrative lives in the folder index [README.md](./README.md).
...
- [package-layout.md](./package-layout.md) — the platform (`src/platform/`) and engine packages at `src/engines/<module>`.
