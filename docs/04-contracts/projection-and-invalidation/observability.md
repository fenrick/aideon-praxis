...
The signals tracked when a projection lags, fails to refresh, or is misused, and the error code each raises. These are
surfaced to the local diagnostic log ([LOGGING_FRAMEWORK.md](../../LOGGING_FRAMEWORK.md)); they carry the
`correlation_id` of the originating write ([ipc/correlation-and-tracing.md](../ipc/correlation-and-tracing.md)).
...
Projection lag is the signal that the consistency model's eventual-convergence promise is taking too long: a `stale`
[freshness state](./freshness-states.md) that outlives its budget is no longer acceptable eventual consistency, it is a
fault worth surfacing.

## References & standards

- **OpenTelemetry**; W3C **Trace Context** _(normative: the correlation these signals carry, via
