...
`ValidTime` is a simpler `i64` wrapper (microseconds since epoch, no counter) used for explicit valid-time stamps on individual facts. It is not an HLC; it records *when an event is true in the world*, not *when it was asserted*.
