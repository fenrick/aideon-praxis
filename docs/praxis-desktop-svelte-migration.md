...
| **Search & sidebar wiring**                                | `Sidebar.svelte`, `searchStore` integration from `+page.svelte`                     | Partial — command palette in React               | React sidebar is static, but the dashboard card now exposes a shadcn command palette (⌘K / Ctrl+K) for branches/commits/actions. Global catalogue/meta-model search remains Svelte-only.                                                                                             |
...
| **Worker/time controls**                                   | `timeStore`, toolbar actions (branch select, commit select, merge)                  | Partial — merge/apply actions missing            | TimeCursorCard replicates branch/commit selection, but merge/apply actions and the timeline slider remain Svelte-only.                                                                                                                       |

## Review snapshot — 15 Nov 2025

- `app/PraxisCanvas/src/components/dashboard/time-cursor-card.tsx` now handles branch + commit selection via shadcn Select components, keeping Svelte’s `Toolbar.svelte` alive only for streaming status messages.
