# Praxis Desktop – Svelte → React Migration Tracker (Historical)

## Purpose

Track the remaining Svelte-based surface area in Praxis Desktop and the steps required to fully
replace it with the React/Tauri Praxis Canvas runtime. This document is a migration tracker; new UX
and feature work must target the React canvas and Aideon Design System.

The React/shadcn canvas in `app/PraxisCanvas` now delivers the primary shell (sidebar, canvas,
catalogue/matrix/chart widgets, templates), but the legacy SvelteKit renderer under
`app/PraxisDesktop` is still bundled. This doc captures the remaining Svelte surface area and the
steps required to fully replace it with the React runtime specified in the implementation guide. The
helper windows (splash, about, status, settings) plus the workspace tabs (overview, timeline,
activity) must be ported before we delete Svelte-specific routes.
...
| **Workspace views** (overview, timeline, canvas, activity) | `MainView.svelte` (+ helpers) orchestrates tabs + ELK canvas + activity feed        | Partial — tabs routed in React                   | `WorkspaceTabs` now cycles between overview metrics, the timeline list, the canvas runtime, and the activity diagnostics card, matching `MainView`’s tabs so the S layout can be retired once the remaining sidebar/search wiring is ported.             |
...
| **Search & sidebar wiring**                                | `Sidebar.svelte`, `searchStore` integration from `+page.svelte`                     | Partial — quick jump now React-powered           | React now owns a `SearchBar` that feeds the same search store (sidebar, commits, catalogue entries) plus the command palette, so quick-jump filtering & navigation behave the same. The Svelte sidebar tree still needs porting before we can delete it. |
...
- `app/PraxisCanvas/src/components/workspace-tabs.tsx` now renders the overview, timeline, canvas, and activity panels originally gated behind `MainView.svelte`, so the S view can be deleted once sidebar/search + inspector parity is reached.
- `app/PraxisCanvas/src/components/blocks/commit-timeline-list.tsx` powers the shared commit list used by both the new timeline tab and the Activity timeline card so the experience mirrors the old S timeline.
