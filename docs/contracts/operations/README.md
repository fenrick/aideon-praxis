...
| 6    | `or-set-update`               | _Deferred to M6 — CRDT convergence op; reserved, not schema'd, not an M0-valid kind (see below)._ |
| 7    | `counter-update`              | _Deferred to M6 — CRDT convergence op; reserved, not schema'd, not an M0-valid kind (see below)._ |
| 8    | `upsert-metamodel-batch`      | [upsert-metamodel-batch.schema.json](./upsert-metamodel-batch.schema.json)                        |
| 9    | `create-scenario`             | _Deferred to a later increment (scenarios are M2); reserved, not schema'd (see below)._           |
| 10   | `delete-scenario`             | _Deferred to a later increment (scenarios are M2); reserved, not schema'd (see below)._           |
