...
function tokenVariable(tokenPath) {
...
function createDictionary(sourceDirectory, semanticFile) {
...
function originalReference(token) {
  const value = token.original.$value;
  const match = typeof value === 'string' ? /^\{([^}]+)\}$/.exec(value) : undefined;
  return match?.[1];
}

function compositeReference(value) {
...
function isTypographyComposite(value) {
...
function renderTypographyValue(token) {
  const value = token.original.$value;
  if (!isTypographyComposite(value)) return undefined;
...
function renderReferenceDeclarations(tokens) {
  return tokens
...
function renderSemanticDeclarations(tokens) {
  return tokens
    .filter((token) => token.path[0] !== 'reference')
    .map((token) => {
...
function renderAliases(aliases) {
  return Object.entries(aliases).map(([name, tokenPath]) =>
    declaration(name, `var(${tokenVariable(tokenPath)})`),
  );
}

function renderCompatibilityAliases() {
  return Object.entries(compatibilityAliases).map(([name, target]) =>
    declaration(name, `var(${target})`),
  );
}

function renderCss(lightTokens, darkTokens) {
  const light = [
...
function renderTypeScript(tokens) {
  return `// Generated by tools/generate-tokens.mjs. Do not edit.
