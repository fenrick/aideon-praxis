...
function parseRefs(subject) {
  const closers = [];
  const refs = [];
  const reClose = /(close[sd]?|fix(e[sd])?|resolve[sd]?)\s+#(\d+)/gi;
  let m;
  while ((m = reClose.exec(subject))) closers.push(Number(m[3] || m[2] || m[1]));
  const reRef = /#(\d+)/g;
  while ((m = reRef.exec(subject))) refs.push(Number(m[1]));
  const unique = (arr) => Array.from(new Set(arr));
  return { closers: unique(closers), refs: unique(refs) };
}

function issueState(num) {
  const j = JSON.parse(gh(['api', `repos/${REPO}/issues/${num}`]));
  return j.state;
}
      ...
      try {
        if (issueState(n) === 'open') {
          ...
          gh([
            ...
            REPO,
            '--comment',
            `Closed via commit ${c.sha.slice(0, 7)} (${url})`,
          ]);
        }
      } catch {}
    }
