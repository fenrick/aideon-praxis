...
const SINCE = getArg('--since', new Date(Date.now() - 120*24*3600*1000).toISOString().slice(0,10));
...
function ensureSnap() { fs.mkdirSync(path.dirname(SNAP), { recursive:true }); if (!fs.existsSync(SNAP)) fs.writeFileSync(SNAP, JSON.stringify({items:{}}, null, 2)); }
function loadSnap() { ensureSnap(); return JSON.parse(fs.readFileSync(SNAP,'utf8')); }
function saveSnap(s) { fs.writeFileSync(SNAP, JSON.stringify(s,null,2)); }

function listIssuesAll() {
  const [owner,name]=REPO.split('/');
  const acc=[]; let page=1; const per=100;
  for(;;){
    ...
    if(!arr.length) break; acc.push(...arr); if(arr.length<per) break; page++;
  }
  return acc.map(i=>({
    number:i.number,
    title:i.title,
...
function listCommits() {
  const out = sh('git', ['log', REF, `--since=${SINCE}`, '--no-merges', `--max-count=${LIMIT}`, '--pretty=format:%H%x09%ad%x09%s', '--date=iso-strict']);
...
const STOP = new Set(['a','an','the','and','or','for','of','to','in','v1','v2','m0','m1','m2','m3','m4','m5','m6','ci','docs']);
...
function score(subjectTokens, titleTokens){
  const s=new Set(subjectTokens); let hit=0; for(const t of titleTokens){ if (s.has(t)) hit++; }
...
function pathTags(files){
  ...
  for(const f of files){
    ...
    if(f.includes('electron')||f.includes('preload')||f.includes('renderer')) tags.add('area/ui');
...
function chooseIssue(commit, issues) {
  ...
  for(const it of issues){
    ...
    let boost=0; for(const t of tags){ if(lbl.has(t)) boost += 0.15; }
    const total = s + boost;
    if(total>best.score){ best={num:it.number, score:total, why:`overlap=${s.toFixed(2)} boost=${boost.toFixed(2)}`}; }
  }
...
try {
  ...
  for(const c of listCommits()){
    considered++;
    if (snap.items && Object.values(snap.items).some(v => (v.shas||[]).includes(c.sha))) continue;
