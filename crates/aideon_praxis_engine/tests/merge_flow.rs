use aideon_mneme_core::temporal::{CommitChangesRequest, MergeRequest, StateAtArgs};
...
async fn merge_returns_conflict_when_branches_diverge_on_same_node() {
    ...
    let target_node = aideon_mneme_core::temporal::NodeVersion {
        id: "conflict-node".into(),
        r#type: Some("Capability".into()),
    ...
    engine
        .commit(CommitChangesRequest {
            ...
            tags: vec![],
            changes: aideon_mneme_core::temporal::ChangeSet {
                node_deletes: vec![aideon_mneme_core::temporal::NodeTombstone {
